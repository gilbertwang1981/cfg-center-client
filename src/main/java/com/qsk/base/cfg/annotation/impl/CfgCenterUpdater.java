...
import com.qsk.base.cfg.consts.CfgServiceConsts;
import com.qsk.base.cfg.utils.HttpUtils;
							...
							try {								
								...
								if (!Strings.isBlank(response)) {
									if (config.getField().getType().getName().equalsIgnoreCase(String.class.getName())) {
										...
										config.getField().set(entry.getValue() , Boolean.valueOf(response.trim()));
									} else {
